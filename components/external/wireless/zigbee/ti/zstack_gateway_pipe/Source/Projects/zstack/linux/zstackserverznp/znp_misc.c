									 ...
									 uint8 * pData)
{
	...
	if (pRsp)
	{
		...
		if ((cmdID == rspcmdid) && (rsplen > 0))
        {
            ...
            {
                uiPrintfEx (trDEBUG,
                            "--> status failed: cmdID:%x, status: %d\n",
                            cmdID, status);
            }
        }
					...
					uint16 len, uint8 * pMsg)
{
	...
	if (subSys == MT_RPC_SYS_ZDO)
	{
		switch (cmdID)
        {
            ...
            case MT_ZDO_NWK_DISCOVERY_CNF:
                ...
                uiPrintfEx (trDEBUG,
                            "znp_misc handleAsyncMsgs: not implemented cmdID:%x\n",
                            cmdID);
                break;
            ...
            case MT_ZDO_STATUS_ERROR_RSP:
                uiPrintfEx (trDEBUG,
                            "znp_misc handleAsyncMsgs: Un-handled cmdID:%x\n",
                            cmdID);
                break;

            default:
                uiPrintfEx (trDEBUG,
                            "znp_misc handleAsyncMsgs: Unrecognized cmdID:%x\n",
                            cmdID);
                break;
        }
	}
            ...
            case MT_SYS_RESET_IND:
                {
                    ...
                    ZDO_RegisterForZDOMsg (zspbTaskID,
                                            ZDO_ALL_MSGS_CLUSTERID);
	...
	{
		uiPrintfEx (trDEBUG,
					"znp_misc handleAsyncMsgs: Unrecognized subSys: %x, cmdID:%x\n",
					subSys, cmdID);
	}
}
...
static void processZDOSourceRouteInd (uint16 len, uint8 * pMsg)
{
	...
	{
		srcRtg.pRelayList =
			(uint16 *) malloc (sizeof (uint16) * srcRtg.relayCnt);
...
static void processAFIncomingMsgInd (uint8 cmdID, uint16 len, uint8 * pMsg)
{
	...
	pBuf = (afIncomingMSGPacket_t *) osal_msg_allocate ((sizeof
	...
	if (pBuf)
	{
		...
		if (cmdID == MT_AF_INCOMING_MSG_EXT)
        {
            ...
            {
                pBuf->srcAddr.addr.shortAddr =
                    BUILD_UINT16 (pMsg[0], pMsg[1]);
            }
            pMsg += 8;
...
static void processZDOMsgCB (uint16 len, uint8 * pMsg)
{
	...
	pBuf =
		(zdoIncomingMsg_t *) osal_msg_allocate (sizeof (zdoIncomingMsg_t) +
												asduLen);
	if (pBuf)
	{
...
uint8 osal_nv_read (uint16 id, uint16 ndx, uint16 len, void *buf)
{
	...
	pRsp =
		apicSendSynchData (ZNP_API_CLIENT, MT_RPC_SYS_SYS, MT_SYS_OSAL_NV_READ,
...
uint8 osal_nv_write (uint16 id, uint16 ndx, uint16 len, void *buf)
{
	...
	if (pBuf)
	{
		...
		*pPtr++ = len;
		...
		status =
			sendNPIExpectDefaultStatusSYS (MT_SYS_OSAL_NV_WRITE, (len + 4),
											 pBuf);
...
{
	return (sendNPIExpectDefaultStatusZNP
			(MT_RPC_SYS_SYS, MT_SYS_SET_TX_POWER, 1, &txPwr));
...
uint8 znpZDOSetParam (uint8 has_useMultiCast, uint8 useMultiCast)
{
	...
	return (sendNPIExpectDefaultStatusZNP
			(MT_RPC_SYS_ZDO, MT_ZDO_EXT_SET_PARAMS, 1, buf));
}
