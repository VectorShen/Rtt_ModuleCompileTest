...
int main (int argc, char *argv[])
{
	...
	{
		uiPrintf ("\nUsage:\n\n");
		uiPrintf ("  %s server_name config.ini\n\n", argv[0]);
		uiPrintf ("Example:\n\n");
		uiPrintf ("  $ %s srv_npi_ipc config.ini\n\n", argv[0]);
		exit (-1);
	}
	...
	if (argc > (pAPIS_SysParams->numClients + 1))
	{
        ...
        {
            if (parseConfigFile
                ((char *)argv[(pAPIS_SysParams->numClients + 1)],
                    pAPIS_SysParams->pConfigDesc,
                    pAPIS_SysParams->numConfigDescs) != 0)
            {
					 ...
					 uint32_t * value, int *argc_p, char *argv[])
{
	...
	for (i = 1; i < *argc_p; i++)
	{
		...
		if ((strcmp (argv[i], flagLong) == 0)
			|| (strcmp (argv[i], flagShort) == 0))
        {
            if ((new_i + 1) >= *argc_p)
            {
                uiPrintf
                    ("ERROR: %s was specified without a value\n. Aborting now.",
                        argv[i]);
                exit (-1);
            }
            ...
            *value = strtoul (argv[new_i + 1], &endptr, 0);
            ...
            {
                uiPrintf
                    ("ERROR: Value following %s has invalid character(s): \"%s\"\n. Aborting now.",
                        argv[i], endptr);
                exit (-1);
            }
